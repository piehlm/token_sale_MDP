...
contract('MDPToken', function(accounts) {
	...
	it('handles delegated transfer', function() {
		return MDPToken.deployed().then(function(instance) {
	    	...
	    	assert(error.message.indexOf('revert') >= 0, 'cannot transfer a value larger than balance');
			return tokenInstance.transferFrom(fromAccount, toAccount, 20, { from: spendingAccount });
	    }).then(assert.fail).catch(function(error) {
	    	assert(error.message.indexOf('revert') >= 0, 'cannot transfer a value larger than approved amount');
